...
--
-- Hôte : 127.0.0.1:3305
-- Généré le : jeu. 18 mars 2021 à 15:48
...
--
CREATE DATABASE IF NOT EXISTS `gescom_afpa` DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci;
USE `gescom_afpa`;
...
,`pro_ref` varchar(8)
,`pro_stock` int unsigned
,`pro_stock_alert` int unsigned
,`pro_sup_id` int unsigned
...
(25, 25, '101.10', 'PRS-01C', NULL, 5, 5, 'Brun', 'Biarritz', 'Quisque fermentum, dui eu elementum sagittis, risus lorem placerat ipsum, vitae venenatis tellus sapien id nibh. Suspendisse et aliquet tellus, in suscipit magna.', 0, 2, '2018-08-19 00:00:00', NULL, 'jpg'),
(26, 25, '612.06', 'PRS-38A', NULL, 4, 5, 'Rose', 'Cannes', 'Curabitur sodales sem vitae ex commodo, in ullamcorper quam congue. Aliquam erat volutpat. Praesent mollis at velit eu molestie. Proin ac tellus a enim venenatis ultrices vitae sed libero. Vivamus at vulputate orci. Curabitur mattis ac turpis id tempus. Sed turpis enim, egestas ac arcu et, elementum luctus ante.', 0, 2, '2018-08-12 00:00:00', NULL, 'jpg'),
(27, 25, '181.95', 'PRS-87F', NULL, 21, 5, 'Rouge', 'Crotoy', 'Morbi porta ultricies nibh vel varius. Vestibulum nec rutrum ex, vel posuere nisi. Ut scelerisque sit amet ligula sed imperdiet. Morbi lacinia sapien in elementum iaculis.', 0, 2, '2018-04-12 00:00:00', '2018-08-21 00:00:00', 'jpg'),
...
DROP VIEW IF EXISTS `catalogue`;
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `catalogue`  AS  select `products`.`pro_id` AS `pro_id`,`products`.`pro_cat_id` AS `pro_cat_id`,`products`.`pro_price` AS `pro_price`,`products`.`pro_ref` AS `pro_ref`,`products`.`pro_ean` AS `pro_ean`,`products`.`pro_stock` AS `pro_stock`,`products`.`pro_stock_alert` AS `pro_stock_alert`,`products`.`pro_color` AS `pro_color`,`products`.`pro_name` AS `pro_name`,`products`.`pro_desc` AS `pro_desc`,`products`.`pro_publish` AS `pro_publish`,`products`.`pro_sup_id` AS `pro_sup_id`,`products`.`pro_add_date` AS `pro_add_date`,`products`.`pro_update_date` AS `pro_update_date`,`products`.`pro_picture` AS `pro_picture`,`categories`.`cat_id` AS `cat_id`,`categories`.`cat_name` AS `cat_name`,`categories`.`cat_parent_id` AS `cat_parent_id` from (`products` join `categories` on((`categories`.`cat_id` = `products`.`pro_cat_id`))) ;
